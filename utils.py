...
def _load_data(dataset_str):
    ...
    N = adj.shape[0]
    TrainLen, TrainTestLen = int(N * 0.6), int(N * 0.8)
    idx_train = range(TrainLen)
    idx_test = range(TrainLen, TrainTestLen)
    idx_val = range(TrainTestLen, N)
...
if __name__ == '__main__':
    ...
    (adj, features, y_train, y_val, y_test,
     train_mask, val_mask, test_mask) = _load_data('cora')
    pdb.set_trace()
